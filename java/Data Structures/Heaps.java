...
class Main {
    public static void main(String[] args) {
        int [][] x = {
            ...
            {19, 36, 17, 12, 25, 100, 5, 9, 15, 30, 6, 11, 13, 8, 1, 304}
        };

        Heaps heap = new Heaps(x[0]);
        heap.HeapSort();
    }
}
...
public class Heaps {
    ...
    protected void buildMaxHeap(int pseudoLength) {
        for (int i = iParent(pseudoLength - 1); i>=0; i--) {
            /* Siftdown. But now, if the root node contains a leftchild we also
             * check if its index is less than the "lastNode" in the Heap. This
             * extra condition opts for heap sorting where the heaps max
             * element is shifted with the lastNode. This lastNode may or may
             * not be the true last node of the heap and is sort of a hack that
             * gives an impression that the max element of the heap has been
             * removed.
             */
