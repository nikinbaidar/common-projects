...
def extract_highlights(sioyek_path,
                       local_database,
                       shared_database,
                       file_path):
    ...
    with sqlite3.connect(local_database):
        ...
        new_file_path = str(pathlib.Path(doc_dir) /
                            new_file_name).replace('\\', '/')
        os.system("lehte")

        highlights = sioyek.get_document(
            doc_path).get_highlights_with_time_stamps()

        with open("/tmp/temporary.txt") as f:
            potential_filename = f.readline()
            potential_filename = potential_filename[:-1]
            os.system("rm /tmp/temporary.txt")

        if potential_filename:
            new_file_path = potential_filename

        if highlights:
            cmd = f"grep -oP '\\d{{4}}-\\d{{2}}-\\d{{2}} \\d{{2}}:\\d{{2}}:\\d{{2}}' '{
                new_file_path}' | tail -1"
            ...
            try:
                ...
                most_recent_extraction = datetime.strptime(
            ...
            delta = timedelta(hours=5, minutes=44, seconds=59)
            document_highlights = [
                (datetime.strptime(x[5], date_format), x[4], x[2]) for x in highlights]
...
def main():
    if len(sys.argv) > 1:
        ...
        extract_highlights(SIOYEK_PATH, LOCAL_DATABASE_FILE,
                           SHARED_DATABASE_FILE, file_path)
